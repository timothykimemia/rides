...
//        $this->db->where('gender' , $gender);
    ...
    public function insert_transaksi_mfood($data_req, $data_detail, $data_messagean){
        ...
        if($this->db->affected_rows() == 1){
            ...
            $arr_messagean = array();
            foreach ($data_messagean as $row){
                $data_messagean = array(
                ...
                array_push($arr_messagean, $data_messagean);
            }
            
            $ins_makanan = $this->insert_barang_belanja_mfood($arr_messagean);
            if($ins_makanan){
                return array(
                    'status'=> true,
                    'data' => $get_data->result()
    ...
    public function insert_transaksi_mstore($data_req, $data_detail, $data_messagean){
        $ins_trans = $this->db->insert('transaksi', $data_req);
        if($this->db->affected_rows() == 1){
            $get_data = $this->get_data_transaksi($data_req);
            $data_hist = array(
...
//            $get_data_mmart = $this->get_data_transaksi_mmart($get_data->row('id'));
            ...
            $ins_barang = $this->insert_barang_belanja_mstore($arr_messagean);
            if($ins_barang){
                return array(
                    'status'=> true,
                    'data' => array(
    ...
    public function insert_transaksi_mmart($data_req, $data_detail, $data_messagean){
...
//            $get_data_mmart = $this->get_data_transaksi_mmart($get_data->row('id'));
            ...
            $ins_barang = $this->insert_barang_belanja_mmart($arr_messagean);
            if($ins_barang){
                return array(
                    'status'=> true,
                    'data'=> $get_data->result()
